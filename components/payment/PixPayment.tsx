...
export function PixPayment({
  ...
  const expiredByClock = expiresAt > 0 && expiresAt <= (now ?? 0);
        ...
        <div className="mt-6 flex flex-col gap-2 sm:flex-row">
          {isClosed || expiredByClock ? (
            <button
              ...
              onClick={regenerate}
              className="flex-1 rounded-xl bg-[#5b2333] px-4 py-3 text-sm font-medium text-white disabled:opacity-50"
            >
              {actionLoading ? "Gerando..." : "Gerar novo PIX"}
            </button>
          ) : (
            <button
              disabled={actionLoading}
              onClick={cancel}
              className="flex-1 rounded-xl border border-red-200 px-4 py-3 text-sm font-medium text-red-700 disabled:opacity-50"
            >
