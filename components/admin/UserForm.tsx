  ...
  async function submit(e: FormEvent) {
    ...
    try {
      const response = await fetch(
        editing ? `/api/admin/usuarios/${user?._id}` : "/api/admin/usuarios",
        {
      ...
      >
        <div className="bg-[#5b2333] px-7 py-5 text-[#f5e6d3]">
          ...
          <p className="mt-1 text-xs text-[#d4a5a5]">
            Gerencie os dados e permissões da conta.
          </p>
        </div>
        <form onSubmit={submit} className="space-y-4 p-7">
          ...
          <label className="block text-xs font-medium text-gray-500">
            ...
            <input
              required
              minLength={3}
              className="mt-1 w-full rounded-xl border border-[#e8ddd0] px-4 py-2.5 text-sm"
              value={form.name}
          ...
          <div className="flex justify-end gap-3 border-t border-[#f0e8e0] pt-5">
            <button
              type="button"
              onClick={onCancel}
              className="rounded-full border border-[#e8ddd0] px-5 py-2.5 text-sm"
            >
              Cancelar
            </button>
            <button
              disabled={loading}
              className="rounded-full bg-[#5b2333] px-6 py-2.5 text-sm font-semibold text-[#f5e6d3] disabled:opacity-50"
            >
              {loading
                ? "Salvando..."
                : editing
