...
type CheckoutContextValue = {
  ...
  setAddress: (value: Address) => void; setCpf: (value: string) => void; lookupCep: () => Promise<void>;
  calculateCurrentShipping: () => Promise<void>; applyCoupon: (code: string) => Promise<boolean>; removeCoupon: () => void;
  getDiscount: () => number; getTotal: () => number; submitCheckout: () => Promise<CheckoutResponse>;
};
...
export function CheckoutProvider({ children }: { children: React.ReactNode }) {
  ...
  const applyCoupon = async (code: string) => {
    ...
    try {
      ...
      setCoupon({ code: result.code, type: result.type, value: Number(result.value), discount: Number(result.discount ?? 0) });
      return true;
    } catch (error) {
      setCoupon(null); setCouponError(error instanceof Error ? error.message : "Cupom inválido."); return false;
  ...
  const getTotal = () => {
    ...
    if (coupon?.type === "shipping") return Math.max(0, Number((getTotalPrice() + Math.max(0, shipping - discount)).toFixed(2)));
    return Math.max(0, Number((getTotalPrice() + shipping - discount).toFixed(2)));
  };
  ...
  const submitCheckout = async () => {
    ...
    try {
      ...
      setCheckoutError(message); throw error;
