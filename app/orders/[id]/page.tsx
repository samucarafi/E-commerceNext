...
export default function OrderDetailsPage() {
  ...
  if (loading) {
    return <main className="min-h-screen bg-[#f8f5f2] p-10 text-center">Carregando pedido...</main>;
  }
    ...
    return (
      <main className="min-h-screen bg-[#f8f5f2] px-4 py-10">
        <div className="mx-auto max-w-3xl rounded-2xl bg-white p-8 text-center">
          ...
          <Link href="/orders" className="mt-5 inline-flex text-sm font-semibold text-[#5b2333]">
            Voltar para pedidos
          </Link>
        </div>
      </main>
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-5 border-b pb-6 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <OrderStatusBadge status={order.deliveryStatus} kind="delivery" />
            </div>
          </div>

          <section className="py-6">
            <div className="mb-4 flex items-center gap-2">
              <Package size={19} className="text-[#9a7b36]" />
              <h2 className="font-semibold">Itens</h2>
            </div>

            <div className="divide-y">
              {productItems.map((item, index) => (
                <div key={item._id ?? `${item.productId}-${index}`} className="flex items-center justify-between gap-4 py-4">
                  <div>
                    <p className="font-medium">{item.title || "Produto"}</p>
                    <p className="text-sm text-gray-500">Quantidade: {item.quantity ?? 0}</p>
                  </div>
