...
export default function CheckoutPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const response = await fetch("/api/payment/pix", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId: result.orderId }) });
      const payment = await response.json();
      if (!response.ok) throw new Error(payment?.error || "Não foi possível gerar o pagamento PIX.");
      // Só limpa o carrinho depois que o pedido e o PIX foram criados com sucesso.
      clearCart();
